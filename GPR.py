...
def main():
    ...
    save_metrics_csv(
        ...
        extra_dict={
            "method": "All_raw_features_to_GPR",
            "n_samples": len(y),
    ...
    print("使用全部数据拟合 All raw features -> GPR 模型......")
    full_model = Pipeline([
        ("imputer", SimpleImputer(strategy="median")),
        ("scaler", StandardScaler()),
        ("gpr", build_gpr()),
    ...
    aux_full_pca = PCA(n_components=PCA_VAR_THRESHOLD, svd_solver="full")
    aux_full_pca.fit(X_all_scaled)

    aux_full_pc_summary_df, aux_full_loading_wide_df, aux_full_loading_long_df, _, _ = pca_to_tables(aux_full_pca, feature_cols)
    aux_full_pc_summary_df["used_by_gpr"] = False

    aux_full_pc_summary_df.to_csv(
        os.path.join(BASE_DIR, f"{SCRIPT_TAG}_aux_full_pc_summary.csv"),
        index=False,
        encoding="utf-8-sig",
    )
    print(f"已保存: {SCRIPT_TAG}_aux_full_pc_summary.csv")

    aux_full_loading_wide_df.to_csv(
        os.path.join(BASE_DIR, f"{SCRIPT_TAG}_aux_full_pc_loadings_wide.csv"),
        index=False,
        encoding="utf-8-sig",
    )
    print(f"已保存: {SCRIPT_TAG}_aux_full_pc_loadings_wide.csv")

    aux_full_loading_long_df["used_by_gpr"] = False
    aux_full_loading_long_df.to_csv(
        os.path.join(BASE_DIR, f"{SCRIPT_TAG}_aux_full_pc_loadings_long.csv"),
        index=False,
        encoding="utf-8-sig",
    )
    print(f"已保存: {SCRIPT_TAG}_aux_full_pc_loadings_long.csv")
    ...
    artifact = {
        ...
        "file_path": FILE_PATH,
        "method": "All_raw_features_to_GPR",
        "auxiliary_pca_for_interpretation": aux_full_pca,
    }
